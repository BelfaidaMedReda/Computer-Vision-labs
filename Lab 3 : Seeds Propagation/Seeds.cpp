...
static float correl(const Image<octet>& im1, int i1,int j1,float m1,
    ...
    float var_1=0.0f;
    float var_2=0.0f;
    for (int offx =-win; offx <= win; offx++) {
        for (int offy=-win; offy <= win; offy++) {
            float pix1 = im1(i1+offx, j1+offy) ;
            float pix2 = im2(i2+offx, j2+offy) ;
            cov += (pix1 - m1) * (pix2 - m2);
            var_1 += pow(pix1 - m1, 2);
            var_2 += pow(pix2 - m2, 2);
        }
    }
    dist = cov / (sqrt(var_1) * sqrt(var_2) + EPS);
    return dist;
}
...
static void find_seeds(Image<octet> im1, Image<octet> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            // We just ignore windows that are not fully in image
            float bestNCC = -1.0f;
            int bestdp = dmin - 1;
            for (int dp=dmin; dp <= dmax; dp++) {
                int new_x = x + dp;
                if ( new_x - win < 0 || new_x + win >= im2.width()) {
                    continue;
                }
                float ncc = ccorrel(im1, x, y, im2,new_x,y);
                if (ncc > bestNCC) {
                    bestNCC = ncc;
                    bestdp = dp;
                }
            ...
            if (bestNCC > nccSeed) {
                seeds(x,y) = true;
                disp(x,y) = bestdp;
                Seed s = Seed(x, y, bestdp, bestNCC);
                Q.push(s);
            }
        }
    }
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && x+win<im1.width() && 0<=y-win && y+win<maxy &&
               ! seeds(x,y)) {
                ...
                seeds(x,y) = true;
                disp(x,y) = bestdp;
                Seed new_s= Seed(x, y, bestdp, bestNCC);
                Q.push(new_s);
            }
        }
    }
